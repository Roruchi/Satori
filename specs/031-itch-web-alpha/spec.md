...
- The itch.io alpha build is a restricted manual upload package on an actual itch.io project page with tester-facing content about the game. PWA is disabled for the first alpha unless a later implementation task proves it is needed for save testing.
...
As the developer, I can publish a content-complete restricted itch.io page, upload a reproducible package, and prove the game runs from that page.

**Independent Test**: Page content review, build artifact review, documented packaging command, recorded itch.io page URL, and browser smoke against the itch.io URL.

**Acceptance Scenarios**:

1. **Given** a clean export, **When** packaging runs, **Then** the output can be uploaded to itch.io.
2. **Given** a restricted itch.io page, **When** a tester opens it before launching the game, **Then** the page explains what Satori is, how to play, what the closed alpha covers, known issues, browser save expectations, and how to send feedback.
3. **Given** a restricted itch.io page with the current Web package uploaded as an HTML/browser-playable build, **When** the page is opened in a browser, **Then** the title, new-game, first ritual, first placement, and same-browser reload smoke pass from the itch.io URL.
...
- **FR-011**: The itch.io page MUST include tester-facing content: title, short description, long description, screenshots or key art, controls/how-to-play notes, alpha scope, known issues, browser save guidance, build version, and feedback route.
- **FR-012**: Evidence MUST record that both the page content and the embedded game were reviewed on the actual itch.io page before this spec can be marked Verified.
...
- **ItchProjectPage**: Restricted or draft itch.io project page that explains Satori and hosts the current Satori Web alpha build.
- **ItchPageContent**: Tester-facing page copy, visuals, controls, alpha scope, known issues, save guidance, version, and feedback route.
...
- **SC-006**: Actual itch.io page content is complete enough for a closed-alpha tester to understand the game, controls, scope, known issues, save behavior, and feedback path before pressing play.
