...
- Player inventory is full when a valid recipe is confirmed; craft completion is blocked, no seed is created, recipe tokens remain in the grid, and an inventory-full message is shown.
...
- **EX-002**: Feature MUST provide clear, readable feedback for success, invalid recipe inputs, locked-element inputs, and inventory-capacity failures.
...
- **SC-004**: In playtest validation, testers complete a seed craft from opening the crafting menu to seeing the item in plant inventory in 30 seconds or less on first attempt.
