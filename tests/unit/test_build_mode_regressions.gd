...
func _setup_build_test_context() -> Dictionary:
	...
	game_state._ready()
	game_state.set("selected_biome", BiomeType.Value.STONE)
...
func _cleanup_build_test_context(ctx: Dictionary) -> void:
	for key: String in ["discovery", "growth", "alchemy"]:
...
func test_ku_build_block_on_stone_marks_pending_origin_shrine() -> void:
	...
	var meadow_recipe: SeedRecipe = alchemy.lookup_recipe([GodaiElement.Value.FU])
	assert_not_null(meadow_recipe)
	assert_true(growth.get_pouch().add(meadow_recipe, 1))
	game_state.place_tile_from_seed(Vector2i(6, 0), BiomeType.Value.STONE, false)
	GameState.selected_biome = BiomeType.Value.KU
...
func test_build_project_requires_adjacency_for_new_pending_blocks() -> void:
	...
	GameState.selected_biome = BiomeType.Value.STONE
...
func test_confirming_multi_tile_non_recipe_project_is_blocked() -> void:
	...
	assert_not_null(base_recipe)
	assert_true(growth.get_pouch().add(base_recipe, 3))
	GameState.selected_biome = BiomeType.Value.MEADOW
...
func test_can_start_new_project_while_previous_project_counts_down() -> void:
	...
	assert_false(controller._toggle_build_block(Vector2i(60, 0)))
	...
	assert_false(bool(second_project_tile.metadata.get("is_build_block", false)))
