...
func test_material_spawn_uses_progression_speed_multiplier() -> void:
	...
	assert_false(game_state.has_ready_material_at(Vector2i(1, 0)))
	assert_eq(game_state.evaluate_material_spawns(3.74).size(), 0)
	assert_false(game_state.has_ready_material_at(Vector2i(1, 0)))
	assert_eq(game_state.evaluate_material_spawns(0.01).size(), 0)
