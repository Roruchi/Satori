...
func test_spirit_patterns_are_not_consumed_by_discovery_registry() -> void:
	var registry := DiscoveryRegistry.new()
	var matcher := PatternMatcher.new()
	matcher.set_discovery_registry(registry)

	var pattern := PatternDefinition.new()
	pattern.discovery_id = "spirit_retry"
	pattern.pattern_type = PatternDefinition.PatternType.CLUSTER
	pattern.required_biomes = [BiomeType.Value.FOREST]
	pattern.size_threshold = 2
	matcher.set_patterns([pattern])
	...
	assert_eq(emitted, ["spirit_retry", "spirit_retry"], "Spirit pattern should retry until SpiritPersistence hydrates it")
	assert_false(registry.has_discovery("spirit_retry"), "Generic discovery registry must not consume spirit IDs")
